...
class Env():
    ...
    def init_state(self):
        ...
        sizeFactor = 20
        self.spider = p.loadURDF(urdf_folder, [ 0 , 0 , 0 ] , [ 0 , 0 , 0 , 1 ] , globalScaling=sizeFactor , useFixedBase = True)
...
while True:
    #print("\n[0] Exit | Train [1] | Load [2] | Delete [3] | sandbox [4]")
    #choice = input().strip()
    ...
    # train
        ...
        print("Number of frames per batch [default = 122880] or infinit [i]:")
        choice = input()
        if choice.isdigit():
            frames_per_batch = int(choice)
        elif choice == 'i':
            frames_per_batch = 4000000
        else:
